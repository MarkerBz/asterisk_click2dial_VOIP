...
class asterisk_server(orm.Model):
    ...
    def _get_calling_number(self, cr, uid, context=None):
        ...
        #//++++++++++++++++++
        #raise orm.except_orm(_('!!!'),_('user, ast_server, ast_manager == %s, %s, %s') % (user, ast_server, ast_manager))
        #raise orm.except_orm(_('Asterisk RESPONSE:'),_('ast_manager.Status() == %s') % (ast_manager.Status()))
        
        calling_party_number = False
        
        #//+++++++++++++++++++++
        #calling_party_number = '+79260000001'
        ...
        try:
            ...
            for chan in list_chan.values():
                ...
                #//!!!!!!!!!!!!!!!!! Fix Incoming GROUP Call:
                # 4 = Ring (GROUP call)
                if (
                        chan.get('ChannelState') == '4' and
                        chan.get('ConnectedLineNum') == '<unknown>'
                        and sip_account in chan.get('Data')):
                    _logger.debug("Found a matching Event in 'Ring' state (GROUP call)")
                    calling_party_number = chan.get('CallerIDNum')
                    break
...
class phone_common(orm.AbstractModel):
    ...
    def click2dial(self, cr, uid, erp_number, context=None):
        ...
        #//+++++++++++++++++++++++
        ast_number = erp_number.replace('+', '')
        #raise orm.except_orm(_(''),_('ast_number ==' + ast_number))
